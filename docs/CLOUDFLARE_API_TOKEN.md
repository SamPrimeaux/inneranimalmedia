...
- **AGENT_SAM_DEPLOY_HOOK_URL** — Full Workers Builds **Deploy Hook** URL (`https://api.cloudflare.com/client/v4/workers/builds/deploy_hooks/<id>`). Stored only as a Worker **secret** (the URL is the credential). Used by `POST /api/internal/trigger-workers-build` (auth: `INTERNAL_API_SECRET`) and by `agentsam_hook` commands `trigger:agent_sam_deploy_hook` / `trigger:workers_deploy_hook` during `fireHooks` (e.g. after sandbox/production promote). Set with `npx wrangler secret put AGENT_SAM_DEPLOY_HOOK_URL -c wrangler.production.toml`.
