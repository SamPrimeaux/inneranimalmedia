...
function ImportsView({ projectSlug, onNavigatePath }) {
  ...
  return <div className="pt-page"><div className="pt-page-inner"><CmsCompactHeader title="Imports" actions={<><button className="pt-btn" onClick={() => onNavigatePath(buildPath('pages', projectSlug))}>Pages</button><button className="pt-btn primary" onClick={submit} disabled={busy}>{busy ? 'Starting...' : 'Start import'}</button></>} />{error && <ErrorBox error={error} onRetry={load} />}<section className="pt-card" style={{ padding: 20, marginBottom: 18 }}><div className="pt-main-grid"><div className="pt-import-drop"><strong style={{ color: 'var(--text)' }}>Import source</strong><div className="pt-toolbar" style={{ gridTemplateColumns: '1fr 1fr auto', marginBottom: 0 }}><input className="pt-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Import name" /><input className="pt-input" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} placeholder="Source URL or R2 key" /><button className="pt-btn primary" onClick={submit} disabled={busy}>Create</button></div></div><div className="pt-card" style={{ padding: 18 }}><div className="pt-label">Import guidance</div></div></div></section>{!imports ? <Loading label="Loading imports..." /> : <section className="pt-card pt-list">{imports.map((imp) => <article className="pt-site-row" key={imp.id}><div className="pt-thumb blue">Liquid import</div><div><div className="pt-row-title">{imp.import_name}</div><div className="pt-row-sub">{imp.source_type || 'manual'} · {imp.import_key || imp.id}</div><div className="pt-row-meta"><span className="pt-badge neutral">{imp.status || 'queued'}</span><span>{imp.sections_found || 0} found</span><span>{imp.sections_mapped || 0} mapped</span></div></div><div className="pt-row-actions"><button className="pt-btn primary">Review</button><button className="pt-btn">Map sections</button></div></article>)}{!imports.length && <div style={{ padding: 24, color: 'var(--muted)' }}>No imports yet.</div>}</section>}</div></div>;
}
