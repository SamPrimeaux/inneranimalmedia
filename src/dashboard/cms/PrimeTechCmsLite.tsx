...
import { parseTemplateMeta, resolveTemplatePreviewUrl } from './templatePreview';
import type {
  CmsBootstrapData,
  CmsBootstrapSection,
  CmsSiteRow,
...
function TemplateSkeletonGrid() {
  return (
    <section className="pt-template-skeleton-grid" aria-busy="true" aria-label="Loading templates">
      {Array.from({ length: 6 }, (_, i) => (
        <article className="pt-card pt-template-skeleton" key={i}>
          <div className="pt-skel-block pt-skel-preview" />
          <div className="pt-skel-block pt-skel-line mid" />
          <div className="pt-skel-block pt-skel-line short" />
          <div className="pt-skel-block pt-skel-line" style={{ marginTop: 4 }} />
        </article>
      ))}
    </section>
  );
...
function TemplatesView({ projectSlug, addToPageId, onNavigatePath }) {
  ...
  const openPreview = (t) => setPreviewTemplate(t);
    ...
    <div className="pt-page">
      <div className="pt-page-inner">
        <PrimeTechHeader
          kicker={CMS_KICKER}
          title="Templates"
          copy="Reusable component templates backed by cms_component_templates. Preview labs and loading states here, or instantiate marketing pages at /marketing/{slug}."
          actions={
            <>
        ...
        {!templates ? <TemplateSkeletonGrid /> : (
          <>
            <div className="pt-toolbar" style={{ gridTemplateColumns: 'minmax(220px,1fr) auto' }}>
              <select
                className="pt-select"
              ...
              <button type="button" className="pt-btn" onClick={load}>Refresh</button>
            </div>
            ...
            {filteredTemplates.map((t) => {
              ...
              const canPreview = Boolean(previewUrl) || t.template_type === 'loading_state' || meta.component;
              const previewStyle = t.preview_image_url
                ? { backgroundImage: `url(${t.preview_image_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }
                : undefined;
              return (
                <article className="pt-card pt-template" key={t.id}>
                  <div>
                    <button
                      type="button"
                      className={`pt-template-preview${canPreview ? ' pt-template-preview--live' : ''}`}
                      style={previewStyle}
                      onClick={() => canPreview && openPreview(t)}
                      disabled={!canPreview}
                      aria-label={canPreview ? `Preview ${t.template_name}` : `${t.template_name} preview not available`}
                    >
                      {canPreview ? <span className="pt-template-preview__badge">Preview</span> : null}
                    </button>
                    <div className="pt-row-title">{t.template_name}</div>
                    <div className="pt-row-sub">{t.category || 'General'} · {t.template_type || 'section'}</div>
                    {meta.description ? <div className="pt-row-sub" style={{ marginTop: 8 }}>{String(meta.description)}</div> : null}
                    <div className="pt-badges">
                  ...
                  <div className="pt-actions" style={{ justifyContent: 'flex-start', marginTop: 18, flexWrap: 'wrap' }}>
                    {isMarketingPage ? (
                      <button
                        type="button"
            ...
            {!filteredTemplates.length ? (
              <div className="pt-card" style={{ padding: 24, color: 'var(--muted)', textAlign: 'center' }}>
      ...
      <div className="pt-page" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div className="pt-page-inner" style={{ maxWidth: 'none', margin: 0, flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '20px 28px 0' }}>
            <PrimeTechHeader kicker={CMS_KICKER} title="Theme editor" copy="Loading section data. Live site preview is already connected." actions={null} />
          </div>
