...
function SitesView({ onNavigatePath }) {
  ...
  const [featuredBootstrap, setFeaturedBootstrap] = useState(null);
  ...
  useEffect(() => {
    if (!featured?.slug) {
      setFeaturedBootstrap(null);
      return;
    }
    api(`/api/cms/bootstrap?project_slug=${encodeURIComponent(featured.slug)}`)
      .then((d) => setFeaturedBootstrap(d))
      .catch(() => setFeaturedBootstrap(null));
  }, [featured?.slug]);
  const workspaceSummary = useMemo(() => {
    const totalPages = rows.reduce((n, s) => n + (Number(s.page_count) || 0), 0);
    const pages = featuredBootstrap?.pages || [];
