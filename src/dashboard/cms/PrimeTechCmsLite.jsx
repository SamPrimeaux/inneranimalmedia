...
const CmsStudioEditor = lazy(() =>
  import('./CmsStudioEditor.jsx').then((m) => ({ default: m.CmsStudioEditor })),
);
...
function heroFieldsFromSection(section) {
  const d = parseJson(section?.section_data, {});
  return {
...
function heroFieldsToSectionData(fields) {
  return {
...
function HeroThemePreview({ fields, siteLabel = 'SITE' }) {
  return (
    <div className="pt-theme-hero-preview">
      <div className="pt-theme-hero-copy">
        {fields.eyebrow ? <small>{fields.eyebrow}</small> : null}
        <h2>{fields.headline || 'TIME IS THE REAL HORSEPOWER'}</h2>
        <p>{fields.subheadline || 'Edit hero copy and preview the storefront live.'}</p>
        <div className="pt-theme-hero-actions">
          {fields.primaryCtaLabel ? <button type="button" className="primary">{fields.primaryCtaLabel}</button> : null}
          {fields.secondaryCtaLabel ? <button type="button" className="secondary">{fields.secondaryCtaLabel}</button> : null}
        </div>
      </div>
      <div className="pt-theme-hero-visual">
        {fields.heroImageUrl ? <img src={fields.heroImageUrl} alt="" /> : siteLabel.slice(0, 3).toUpperCase()}
      </div>
    </div>
  );
}

function SitesView({ sites = [], primaryProjectSlug = null, workspaceSlug = null, loadingSites = false, sitesError = '', onNavigatePath, onRetry }) {
  const rows = sites || [];
  ...
  const siteUrl = (site) => {
    if (site?.domain?.startsWith('http')) return site.domain;
    if (site?.domain) return `https://${site.domain}`;
    return `https://${site.slug}.workers.dev`;
  };
  return (
            ...
            {featured ? (
              <section className="pt-card pt-feature">
                <div className="pt-stage">
                  <BrowserPreview title={(featured.name || featured.slug || 'SITE').toUpperCase()} copy={`${featured.page_count || 0} pages connected to PrimeTech CMS Lite.`} />
                </div>
                <aside className="pt-feature-meta">
                  <div>
                    <div className="pt-site-name"><span className="pt-mark">PT</span>{featured.name || featured.slug}</div>
                    <div className="pt-domain">{featured.domain || `${featured.slug}.workers.dev`}</div>
                ...
                <section className="pt-card pt-list">
                  {otherSites.map((site, i) => (
                    <article className="pt-site-row" key={site.slug || site.id}>
                      <div className={`pt-thumb ${i % 3 === 0 ? 'warm' : i % 3 === 1 ? 'blue' : 'green'}`}>PrimeTech · {site.name || site.slug}</div>
                      <div>
                        <div className="pt-row-title">{site.name || `CMS · ${site.slug}`}</div>
                        <div className="pt-row-sub">{site.domain || `${site.slug}.workers.dev`}</div>
...
function OnlineStoreView({ projectSlug, workspaceLabel, onNavigatePath }) {
  const { loading, error, data, reload } = useBootstrap(projectSlug, null);
  const theme = data?.active_theme;
  const pages = data?.pages || [];
  const published = pages.filter((p) => String(p.status).toLowerCase() === 'published').length;
  const publicUrl = data?.tenant?.domain
    ? (String(data.tenant.domain).startsWith('http') ? data.tenant.domain : `https://${data.tenant.domain}`)
    : `https://${projectSlug}.workers.dev`;
  const lastSaved = pages.reduce((latest, p) => {
    const ts = p.updated_at || p.published_at;
    if (!ts) return latest;
    if (!latest) return ts;
    return new Date(ts) > new Date(latest) ? ts : latest;
  }, null);
  if (loading) return <Loading label="Loading online store..." />;
  return (
    <div className="pt-page">
      <div className="pt-page-inner">
        <PrimeTechHeader
        ...
        <section className="pt-card pt-online-active">
          <div className="pt-online-previews">
            <div className="pt-online-desktop"><BrowserPreview title={(workspaceLabel || projectSlug || 'STORE').toUpperCase()} copy="Desktop storefront preview" /></div>
            <div className="pt-online-mobile"><BrowserPreview title="MOBILE" copy="Mobile preview" /></div>
          </div>
          <aside className="pt-online-meta">
            <div>
              ...
              <div className="pt-badges" style={{ marginTop: 12 }}>
                ...
                <span className="pt-badge neutral">{published} live pages</span>
              </div>
            </div>
            <div className="pt-feature-stats">
...
function ThemeEditorView({ projectSlug, workspaceId, workspaceLabel, onNavigatePath }) {
  const { loading, error, data, reload } = useBootstrap(projectSlug, null);
  ...
  if (loading || !fields) return <Loading label="Loading theme editor..." />;
        ...
        <section className="pt-theme-editor" style={{ flex: 1 }}>
          ...
          <div className="pt-theme-preview-wrap">
            ...
            <div className="pt-theme-preview-canvas">
              <HeroThemePreview fields={fields} siteLabel={workspaceLabel || projectSlug || 'SITE'} />
            </div>
          </div>
        </section>
      </div>
...
export function PrimeTechCmsLite({ workspaceId = '', workspaceLabel = null, workspaceSlug = null, sites = [], primaryProjectSlug = null, loadingSites = false, sitesError = '', onRetrySites = () => {}, view = 'sites', projectSlug = null, pageId = null, studioPanel = 'pages', addToPageId = null, loadingProject = false, projectError = '', onNavigatePath = () => {} }) {
  useStyle();
  ...
  return (
    <div className="pt-cms-lite" data-brand="PrimeTech CMS Lite">
      ...
      {view === 'online-store' ? <OnlineStoreView projectSlug={projectSlug} workspaceLabel={workspaceLabel} onNavigatePath={onNavigatePath} /> : null}
      {view === 'theme-editor' ? <ThemeEditorView projectSlug={projectSlug} workspaceId={workspaceId} workspaceLabel={workspaceLabel} onNavigatePath={onNavigatePath} /> : null}
      ...
      {view === 'pages' ? (pageId ? <PageEditor workspaceId={workspaceId} projectSlug={projectSlug} pageId={pageId} onNavigatePath={onNavigatePath} /> : <PagesList workspaceLabel={workspaceLabel} projectSlug={projectSlug} onNavigatePath={onNavigatePath} />) : null}
    </div>
  );
}
