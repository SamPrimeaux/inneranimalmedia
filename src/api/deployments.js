...
import { handleGitStatusApi }                             from './git-status.js';
import { notifySam }                                      from '../core/notifications.js';

// ─── CICD Pipeline Mirror ─────────────────────────────────────────────────────
...
async function evaluateGate(env, gate, context) {
  ...
  try {
    ...
    if (metric_key === 'recent_deploy_failures') {
      ...
      const row    = await env.DB.prepare(`SELECT COUNT(*) AS cnt FROM deployments WHERE status = 'failed' AND datetime(timestamp) > datetime('now', '-1 hour')`).first();
    ...
    if (metric_key === 'error_rate_1h') {
      if (!env.DB) return { actual_value: '0', passed: true };
      const row    = await env.DB.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN severity = 'error' THEN 1 ELSE 0 END) AS errors FROM agent_telemetry WHERE created_at > unixepoch() - 3600`).first();
    ...
    if (metric_key === 'worker_version') {
      ...
      const row    = await env.DB.prepare(`SELECT version FROM deployments WHERE status = 'success' ORDER BY timestamp DESC LIMIT 1`).first();
...
export async function runQualityGates(env, opts = {}) {
  ...
  await env.DB.prepare(
    `UPDATE quality_runs SET status=?, pass_count=?, fail_count=?, warn_count=?, completed_at=unixepoch() WHERE id=?`
  ).bind(runStatus, passCount, failCount, warnCount, runId).run();

  // Write failures into Agent Sam memory for learning
  if (failCount > 0) {
      ...
      await env.DB.prepare(
        `INSERT INTO agent_memory_index
         ...
         ON CONFLICT(key) DO UPDATE SET
           value=excluded.value, importance_score=excluded.importance_score, updated_at=unixepoch()`
      ).bind(
        tenantId,
...
export async function runPostDeployQualityChecks(env, deploymentId, opts = {}) {
  ...
  try {
    ...
    ).bind(deploymentId, JSON.stringify({ triggered_at: new Date().toISOString(), deployment_id: deploymentId })).run();
...
export async function handleDeploymentsApi(request, url, env, ctx) {
    ...
    try {
      const { results } = await env.DB.prepare(
        `SELECT pr.*, COUNT(s.id) AS step_count
         ...
         GROUP BY pr.run_id ORDER BY pr.triggered_at DESC LIMIT ?`
      ).bind(limit).all();
  ...
  if (path === '/api/internal/record-deploy' && method === 'POST') {
    if (!isIngestSecretAuthorized(request, env)) return jsonResponse({ error: 'Unauthorized system access' }, 401);
      ...
      await env.DB.prepare(
        `INSERT INTO deployments
         ...
         VALUES (?, datetime('now'), ?, ?, 'Internal record-deploy (API)', 'success', ?,
                 ?, 0, ?, ?, ?)`
      ).bind(deployId, versionId || deployId, gitHash || null, triggeredBy, environment, workerName, triggeredBy, notes || null).run();

      appendCidiPipelineRunFromDeploy(env, { deploymentId: deployId, environment, gitHash: gitHash || 'unknown', versionId: versionId || deployId, description: notes || 'deploy via script' });
