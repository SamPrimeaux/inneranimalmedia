...
export async function agentChatSseHandler(env, request, ctx, opts = {}) {
  ...
  // Canonical field: `mode`. Legacy aliases kept for one release (old clients / scripts).
  // Prefer body.mode when present; fall back only if omitted.
  const runtimeMode = normalizeAgentRuntimeMode(
    body.mode != null && String(body.mode).trim() !== ''
      ? body.mode
      : body.agent_mode ?? body.runtime_intent_mode ?? body.execution_mode,
  );
