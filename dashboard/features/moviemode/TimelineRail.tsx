...
export const TimelineRail: React.FC<TimelineRailProps> = ({ timeline, onTimelineChange }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 overflow-auto p-2 ${dragging ? 'cursor-grabbing' : ''}`}
        ...
        <div style={{ width: widthPx, minWidth: '100%' }}>
          {timeline.tracks.map((track) => (
            <div key={track.id} className="mb-2">
              ...
              <div
                className="relative h-10 rounded bg-[var(--scene-bg)] border border-[var(--border-subtle)]"
                style={{ width: widthPx }}
              >
                {track.clips.length === 0 && (
                  <span className="absolute inset-0 flex items-center justify-center text-[10px] text-[var(--text-muted)] opacity-50">
                    Drag clips to reposition · add from Media Library
                  </span>
                )}
                {track.clips.map((clip) => {
                  ...
                  return (
                    <button
                      key={clip.id}
                      type="button"
                      className="absolute top-1 bottom-1 rounded px-1 flex items-center gap-0.5 text-[10px] font-medium text-black bg-[var(--solar-cyan)] border border-[var(--solar-cyan)] hover:brightness-110 cursor-grab active:cursor-grabbing overflow-hidden"
                      style={{
