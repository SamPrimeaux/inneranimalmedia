...
import { HealthShell, ANALYTICS_TABS, type HealthTabId } from '../components/health/HealthShell';
...
const TAB_IDS = new Set<string>(ANALYTICS_TABS.map((t) => t.id));

/** `/dashboard/analytics/:tab` — legacy `/dashboard/health` redirects in App routes. */
export const HealthPage: React.FC = () => {
  const navigate = useNavigate();
  const params = useParams<{ tab?: string }>();
  const raw = params.tab?.toLowerCase().trim();
  const tab: HealthTabId = raw && TAB_IDS.has(raw) ? (raw as HealthTabId) : 'overview';
  ...
  const setTab = useCallback(
    (t: HealthTabId) => {
      navigate(`/dashboard/analytics/${t}`);
    },
    [navigate],
  );
  ...
  return (
    <div className="flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden">
      ...
      >
        {loading && !summary ? (
          <div className="ov-inlineNotice">Loading analytics…</div>
        ) : null}
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 border-t border-[var(--border-subtle)] pt-4">
              <div>
                ...
                {!(advisors?.security || []).length ? (
                  <div className="text-[var(--text-muted)] text-sm">No security flags.</div>
                ) : null}
              </div>
              <div>
        ...
        {tab === 'agent' && (
          <div className="space-y-6">
            ...
            <div className="border-t border-[var(--border-subtle)] pt-4">
              <h3 className="text-sm font-semibold mb-2">Models · latency vs cost</h3>
            ...
            <div className="border-t border-[var(--border-subtle)] pt-4">
              ...
              <div className="space-y-2 text-[13px]">
                <div className="text-[var(--text-muted)]">
                  Last success: {deployments?.last_success_at || '—'} · Last failure:{' '}
                  {deployments?.last_failure_at || '—'} · 7d count: {deployments?.deploy_count_7d ?? 0}
                </div>
