...
/**
 * FIX: Path-based immutable tree update.
 *
 * The old `mapFileNodeInTree` used JS reference equality (`node === target`).
 * After any async state update (e.g. `await getEntries()`), the node object
 * captured in the click-handler closure may no longer be the same reference
 * that lives in the latest `prev` state — causing silent no-ops where the tree
 * appears to do nothing when you click a folder.
 *
 * This version walks the tree by reconstructed path string instead, which is
 * stable across re-renders and async boundaries.
 */
function mapFileNodeByPath(
    root: FileNode,
    targetPath: string,
    fn: (n: FileNode) => FileNode,
): FileNode {
    function walk(node: FileNode, parentPath: string): FileNode {
        const nodePath = parentPath ? `${parentPath}/${node.name}` : node.name;
        if (nodePath === targetPath) return fn({ ...node });
        if (!node.children?.length) return node;
        let changed = false;
        const nextChildren = node.children.map((ch) => {
            const nc = walk(ch, nodePath);
...
export const LocalExplorer: React.FC<{
    ...
    /**
     * FIX: toggleDir now uses path-based tree updates instead of reference equality.
     *
     * Old approach called `findNode(rootDir, node)` (reference scan) then
     * `mapFileNodeInTree(prev, node, ...)` inside the state updater.  After the
     * `await getEntries()` call the `node` reference captured at click-time is
     * stale relative to `prev`, so `mapFileNodeInTree` silently found nothing and
     * the tree appeared frozen.
     *
     * New approach:
     *   1. Reconstruct the stable path string from the same `pathPrefix` + `node.name`
     *      that `renderTree` uses for keys — no reference needed.
     *   2. Read `node.isOpen` directly from the rendered node (safe; we only need
     *      the boolean at click-time to decide expand vs. collapse).
     *   3. Pass the path string into `mapFileNodeByPath` which walks by name, not
     *      by object identity — works correctly even after async state updates.
     */
    const toggleDir = useCallback(async (node: FileNode, pathPrefix: string) => {
        if (node.kind === 'file') {
        ...
        // Stable path string — same formula used as React key in renderTree.
        const nodePath = pathPrefix ? `${pathPrefix}/${node.name}` : node.name;

        if (node.isOpen) {
            // Collapse: no async work needed, just flip the flag.
            setRootDir((prev) =>
                prev ? mapFileNodeByPath(prev, nodePath, (n) => ({ ...n, isOpen: false })) : prev,
            );
            return;
        }

        // Expand: load children, then update via path so stale references never matter.
        const children = await getEntries(node.handle);
        setRootDir((prev) =>
            prev ? mapFileNodeByPath(prev, nodePath, (n) => ({ ...n, isOpen: true, children })) : prev,
        );
    }, [getEntries, onFileSelect]);
