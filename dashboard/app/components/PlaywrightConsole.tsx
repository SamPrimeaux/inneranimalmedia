...
import { Globe, Play, RefreshCw, CheckCircle2, Clock, AlertCircle, ExternalLink, Search, Trash2, Camera } from 'lucide-react';
            ...
            <div className="p-4 border-b border-[var(--border-subtle)] bg-[var(--bg-panel)] flex flex-col gap-4">
                ...
                <form onSubmit={handleLaunchJob} className="flex gap-2">
                    <div className="relative flex-1 group">
                        <Globe size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] group-focus-within:text-[var(--solar-cyan)] transition-colors" />
                        <input 
                            type="text" 
                            placeholder="Snapshot URL..."
                            value={targetUrl}
                            onChange={(e) => setTargetUrl(e.target.value)}
                            className="w-full bg-[var(--bg-app)] border border-[var(--border-subtle)] rounded-lg py-2 pl-9 pr-3 text-sm focus:outline-none focus:border-[var(--solar-cyan)]/50 focus:ring-1 focus:ring-[var(--solar-cyan)]/20 transition-all font-mono"
                        />
                    </div>
                    <button 
                        type="submit"
                        disabled={isLaunching || !targetUrl}
                        className="bg-[var(--solar-cyan)] hover:bg-[var(--solar-cyan)]/80 text-[var(--bg-app)] h-10 w-12 flex items-center justify-center rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed group shadow-lg shadow-[var(--solar-cyan)]/10"
                        title="Capture Screenshot"
                    >
                        {isLaunching ? (
                            <RefreshCw size={18} className="animate-spin" />
                        ) : (
                            <Camera size={18} fill="none" strokeWidth={2.5} className="group-hover:scale-110 transition-transform" />
                        )}
                    </button>
                </form>
