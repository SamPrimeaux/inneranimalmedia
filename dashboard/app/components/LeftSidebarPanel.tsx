...
function ActivityBar({ active, onChange, gitChangesCount }: ActivityBarProps) {
  ...
  const mainButtons: { id: PanelId; icon: React.ReactNode; label: string; badge?: number }[] = [
    { id: 'explorer',       icon: <Icon.Explorer/>,      label: 'Explorer' },
    { id: 'search',         icon: <Icon.Search/>,         label: 'Search' },
    { id: 'source-control', icon: <Icon.SourceControl/>, label: 'Source Control', badge: gitChangesCount },
    { id: 'run-debug',      icon: <Icon.RunDebug/>,       label: 'Run and Debug' },
  ];
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 1, padding: '4px 6px', background: 'var(--bg-panel)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
      {mainButtons.map(btn => (
        <button
          key={btn.id} title={btn.label} onClick={() => onChange(btn.id)}
          style={{
...
export default function LeftSidebarPanel(props: LeftSidebarPanelProps) {
  ...
  return (
    ...
    >
      <ActivityBar active={activePanel} onChange={setActivePanel} gitChangesCount={unstagedCount}/>
