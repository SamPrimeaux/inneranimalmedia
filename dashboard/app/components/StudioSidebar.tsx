...
import {
  Gamepad2, Layers, Box, Download, Package, Sparkles, Zap, Mountain, Trees, LayoutGrid, Dumbbell,
  Sun, Moon, Ghost, Plane, Activity, Shield, Palette,
  Plus, Trash2, Link, UploadCloud, Eye, Sword, UserCircle, Globe,
...
const R2_BASE = 'https://pub-e733f82cb31c4f34b6a719e749d0416d.r2.dev';
...
interface SidebarProps {
  ...
  /** When true, sidebar is embedded in the IDE activity column (layout hint). */
  isEmbedded?: boolean;
}
...
const ASSET_GALLERY = [
  { name: 'IAM Footer',        url: `${R2_BASE}/inneranimalmediafooterglb.glb`,                                          icon: <Shield   size={14} />, scale: 1.5 },
  { name: 'Kinetic Symmetry',  url: `${R2_BASE}/Kinetic_Symmetry_0831084700_generate%20(1).glb`,                        icon: <Activity size={14} />, scale: 2   },
  { name: 'Meshy Jet',         url: `${R2_BASE}/Meshy_AI_Jet_in_Flight_0104205113_texture.glb`,                         icon: <Plane    size={14} />, scale: 1.2 },
];

const getChessUrl = (color: 'white' | 'black', piece: string) =>
  `${R2_BASE}/chess/v1/pieces/${color}/${piece}.glb`;
...
export const StudioSidebar: React.FC<SidebarProps> = ({
  ...
  const handleDirectSpawn = () => {
    ...
    if (!url) return;
    onSpawnModel('Remote Asset', url, 1);
    setDirectUrl('');
  };
  ...
  const toggleAdding = () => setIsAdding((v) => !v);
  ...
  return (
    <div className="w-80 h-full bg-[var(--bg-panel)] border-r border-[var(--border-subtle)] flex flex-col p-5 z-20 overflow-y-auto custom-scrollbar">
      ...
      <div className="space-y-8 flex-1 pb-10">
              ...
              >
                <div className={`mt-1 p-2 rounded-lg transition-colors ${
                  activeProject === p.id
                    ...
                    : 'bg-white/5 text-[var(--text-muted)] group-hover:text-white'
                }`}>
                ...
                <div className="flex-1">
                  <div className={`text-[12px] font-bold tracking-tight transition-colors ${
                    activeProject === p.id ? 'text-[var(--solar-cyan)]' : 'text-white/80'
                  }`}>{p.name}</div>
        ...
        {/* Chess Piece Armory (Chess project only) */}
        {activeProject === ProjectType.CHESS && (
          <section className="bg-white/5 p-5 rounded-2xl border border-indigo-500/20 space-y-4 animate-in fade-in slide-in-from-left duration-500">
            ...
            {(['white', 'black'] as const).map((color) => (
              <div key={color}>
                <p className="text-[9px] font-bold text-white/20 uppercase tracking-tighter mb-2">
                  {color === 'white' ? 'White Pieces' : 'Black Pieces'}
                </p>
                <div className="grid grid-cols-3 gap-2">
                  {CHESS_PIECES.map((piece) => (
                    <button
                      key={`${color}-${piece.type}`}
                      onClick={() => onSpawnModel(`${color === 'white' ? 'White' : 'Black'} ${piece.name}`, getChessUrl(color, piece.type), 0.8)}
                    ...
                    >
                      <div className={color === 'white' ? 'text-white/60 group-hover:text-white' : 'text-indigo-400 group-hover:text-indigo-300'}>
                        <UserCircle size={16} />
                      </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
              ...
              <button
                ...
                aria-pressed={sceneConfig.castShadows}
              >
            ...
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
              ...
              <button
                ...
                aria-pressed={sceneConfig.showPhysicsDebug}
              >
        ...
        <section className="bg-white/5 p-5 rounded-2xl border border-white/5 space-y-6">
          ...
          <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
            ...
            <button
              ...
              aria-pressed={genConfig.usePhysics}
            >
