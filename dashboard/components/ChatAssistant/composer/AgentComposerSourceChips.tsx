...
import { WEB_SEARCH_SOURCE_ID } from './types';
    ...
    <div className={`flex flex-wrap items-center gap-1.5 min-w-0 ${className}`.trim()}>
      {sources.map((s) => (
        <span
          ...
          title={s.kind === 'web_search' ? 'Web search enabled for this message' : `Using ${s.label}`}
        >
          {chipIcon(s.kind)}
          <span className="truncate">
            {s.id === WEB_SEARCH_SOURCE_ID ? 'Web search' : s.label}
          </span>
          <button
            type="button"
