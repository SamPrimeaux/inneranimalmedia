/**
 * Procedural voxel models for AgentSamEngine (stock shapes / demos).
 * @license SPDX-License-Identifier: Apache-2.0
 */

import { ArtStyle, VoxelData } from '../types';
import { AgentSamSceneColors, AgentSamSceneConfig } from './agentSamConstants';
...
const COLORS = AgentSamSceneColors;
const CONFIG = AgentSamSceneConfig;

export const AgentSamGenerators = {
  Eagle: (): VoxelData[] => {
    ...
    for (let x = EX - 2; x <= EX + 2; x++)
      for (let y = EY + 4; y <= EY + 9; y++) setBlock(map, x, y, EZ + 3, COLORS.LIGHT);
    for (const x of [-4, -3, 3, 4])
      for (let y = EY + 4; y <= EY + 10; y++)
        for (let z = EZ - 2; z <= EZ + 3; z++) setBlock(map, x, y, z, COLORS.DARK);
    for (let x = EX - 2; x <= EX + 2; x++)
      for (let y = EY; y <= EY + 4; y++)
        for (let z = EZ - 5; z <= EZ - 3; z++) setBlock(map, x, y, z, COLORS.WHITE);
    const HY = EY + 12,
      HZ = EZ + 1;
    ...
    for (const o of [
      ...
      setBlock(map, EX + o[0], EY + o[1], EZ, COLORS.TALON);
    for (const o of [
      [0, 1],
      [0, 2],
      [1, 1],
      ...
      setBlock(map, EX + o[0], HY, HZ + 2 + o[1], COLORS.GOLD);
    setBlock(map, EX, HY - 1, HZ + 3, COLORS.GOLD);
    for (const o of [
      [-1.5, COLORS.BLACK],
      [1.5, COLORS.BLACK],
    ] as const)
      setBlock(map, EX + o[0], HY + 0.5, HZ + 1.5, o[1]);
    for (const o of [
      [-1.5, COLORS.WHITE],
      [1.5, COLORS.WHITE],
    ] as const)
      setBlock(map, EX + o[0], HY + 1.5, HZ + 1.5, o[1]);
  ...
  Cat: (): VoxelData[] => {
    ...
    for (const side of [
      ...
      setBlock(map, CX + side[0], CHY + 3, CZ, COLORS.DARK);
      setBlock(map, CX + side[0] * 0.8, CHY + 3, CZ + 1, COLORS.WHITE);
  ...
  Twins: (): VoxelData[] => {
    ...
    function buildMiniEagle(offsetX: number, offsetZ: number) {
      ...
      for (let x = EX - 1; x <= EX + 1; x++)
        for (let y = EY + 2; y <= EY + 6; y++) setBlock(map, x, y, EZ + 2, COLORS.LIGHT);
      for (let x = EX - 1; x <= EX + 1; x++)
        for (let y = EY + 2; y <= EY + 3; y++) setBlock(map, x, y, EZ - 3, COLORS.WHITE);
...
export type AgentSamGeneratorKey = keyof typeof AgentSamGenerators;

export const AGENT_SAM_GENERATOR_KEYS = Object.keys(AgentSamGenerators) as AgentSamGeneratorKey[];

/** Default procedural preset when a viewport art style is selected. */
export const AGENT_SAM_STYLE_GENERATORS: Record<ArtStyle, AgentSamGeneratorKey> = {
  [ArtStyle.CYBERPUNK]: 'Eagle',
  [ArtStyle.ORGANIC]: 'Rabbit',
  [ArtStyle.BRUTALIST]: 'Twins',
  [ArtStyle.LOW_POLY]: 'Cat',
};

export function isAgentSamGeneratorKey(value: string): value is AgentSamGeneratorKey {
  return value in AgentSamGenerators;
}
