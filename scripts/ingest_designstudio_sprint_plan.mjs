#!/usr/bin/env node
/**
 * Design Studio sprint plan → agentsam_documents_oai3large_1536 + AGENTSAM_VECTORIZE_DOCUMENTS.
 * Manifest: docs/platform/designstudio-sprint.manifest.json
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_designstudio_sprint_plan.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_designstudio_sprint_plan.mjs
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_designstudio_sprint_plan.mjs --file docs/platform/designstudio-sprint-plan.md
 */
...
const SCRIPT_KEY = 'ingest_designstudio_sprint_plan';
const MANIFEST = 'docs/platform/designstudio-sprint.manifest.json';
const DOC_TYPE = 'designstudio_sprint_plan';
const TOPIC = 'designstudio_sprint';
...
async function main() {
  ...
  console.log(`ingest_designstudio_sprint_plan — ${args.dryRun ? 'DRY RUN' : 'LIVE'}`);
  ...
  try {
    ...
    console.log(`\nDone — ${totalChunks} total chunks from ${files.length} Design Studio sprint doc(s)`);
  } finally {
