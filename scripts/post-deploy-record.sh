...
npx wrangler d1 execute inneranimalmedia-business --remote --config "$CONFIG" --command "INSERT INTO agentsam_deployment_health (id, tenant_id, deployment_id, worker_name, environment, check_type, check_url, status, http_status_code, response_time_ms, metadata_json, checked_by, checked_at, workspace_id, checked_at_unix, last_checked_at) VALUES ('$HEALTH_ID_ESC', '$TID_ESC', '$VID_ESC', 'inneranimalmedia', 'production', 'smoke_test', 'https://inneranimalmedia.com/api/health', 'ok', NULL, NULL, json_object('git_hash', '$GH_ESC', 'triggered_by', '$TB_ESC', 'deploy_time_seconds', $DEPLOY_SECONDS, 'phase', 'post_deploy_record'), 'post_deploy_record', strftime('%Y-%m-%dT%H:%M:%SZ','now'), '$WID_ESC', unixepoch(), unixepoch())" \
