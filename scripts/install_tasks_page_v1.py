...
TASKS_DIR = ROOT / "dashboard" / "pages" / "tasks"
TASKS_PAGE = TASKS_DIR / "TasksPage.tsx"

if not APP.exists():
    raise SystemExit("dashboard/App.tsx not found. Run this from repo root.")

TASKS_DIR.mkdir(parents=True, exist_ok=True)

TASKS_PAGE.write_text(r'''import React, { useMemo, useState } from "react";
import {
  AlertTriangle,
  ArrowRight,
  Bot,
...
# Add lazy page import/const near existing lazy pages.
tasks_const = "const TasksPage = lazy(() => import('./pages/tasks/TasksPage'));\n"
if "const TasksPage = lazy(() => import('./pages/tasks/TasksPage'));" not in app:
    marker = "const SettingsPanel = lazy(() => import('./components/settings'));\n"
    if marker in app:
        app = app.replace(marker, marker + tasks_const)
    else:
        # Fallback: insert after the last lazy const.
        lines = app.splitlines(True)
        insert_at = 0
        for i, line in enumerate(lines):
            if line.startswith("const ") and "lazy(() => import(" in line:
                insert_at = i + 1
        lines.insert(insert_at, tasks_const)
        app = "".join(lines)

# Add route inside dashboard Routes.
route = '                      <Route path="/dashboard/tasks" element={<TasksPage />} />\n'
if 'path="/dashboard/tasks"' not in app:
    candidates = [
    ...
    for marker in candidates:
        ...
        if idx != -1:
            ...
            app = app[:end + 1] + route + app[end + 1:]
            inserted = True
            break
    if not inserted:
        raise SystemExit("Could not find dashboard routes block to insert /dashboard/tasks.")
...
# Verify built bundle includes the page marker.
grep = subprocess.run(
    ["bash", "-lc", "grep -R \"Workspace task board\\|Awaiting Approval\\|Agent Linked\" -n dashboard/dist dist 2>/dev/null | head -20"],
    text=True,
)
...
run(["git", "add", "dashboard/App.tsx", "dashboard/pages/tasks/TasksPage.tsx"])
run(["git", "commit", "-m", "feat(dashboard): add tasks kanban page"], allow_fail=True)
run(["git", "push", "origin", "main"])

print("\nDone.")
print("Next production step if GitHub/Worker deploy does not upload frontend R2 assets:")
print("  ./scripts/upload-dashboard-app-r2-prod.sh")
print("or:")
