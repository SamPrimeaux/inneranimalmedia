...
const SCRIPT_KEY = 'ingest_repo_skills_rag';
...
function contentHashLocal(text) {
...
function writeRunReceipt(stats, status = 'ok', error = null) {
  ...
  const details = buildReceiptDetails({
    ...
    git_commit_sha: stats.gitSha,
    workspace_id: stats.workspaceKey,
    workspace_uuid: stats.workspaceUuid,
    ...
    files_indexed: stats.skillsCount,
    status,
    error,
  });
  writeVectorizeSyncReceipt({
...
function buildRows(skill, sections, workspaceUuid, gitSha) {
  ...
  return sections.map((sec, i) => {
    ...
    const h = contentHashLocal(`${skill.key}:${i}:${body}`);
...
async function main() {
  ...
  console.log(`run_id: ${runId} git_commit_sha: ${gitSha}`);
  ...
  try {
    ...
    writeRunReceipt({
      ...
      workspaceKey: d1Key,
      workspaceUuid,
      chunksEmbedded: savedRows.length,
