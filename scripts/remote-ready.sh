...
echo ""
echo "Mac-free SHIP (do not run deploy:full on the VM):"
echo "  npm run ship:remote              # push → Cloudflare Builds (Vite+R2+worker)"
echo "  npm run ship:remote -- --worker-only   # emergency wrangler-only"
echo "  npm run deploy:fast              # Mac/CI only — critical path, no post-hooks"
