...
INSERT OR REPLACE INTO ai_knowledge_base (
  ...
  'Themes: Table cms_themes (id, name, slug, config JSON). GET /api/themes lists themes. Config keys: text (--text-nav, --text-primary), textSecondary (--text-secondary), nav, bg, surface, border, primary. To fix dark-theme contrast set config.text and config.textSecondary to light hex (#f1f5f9, #94a3b8). SQL to update: UPDATE cms_themes SET config = json_set(coalesce(json(config), ''{}''), ''$.text'', ''#f1f5f9'', ''$.textSecondary'', ''#94a3b8'') WHERE slug = ?; User refreshes dashboard after update; no deploy. Suggest SQL first, wait for approval. Other components: Dashboard HTML in repo dashboard/*.html; served from R2 inneranimalmedia at static/dashboard/<name>.html. To publish: upload file to R2 then npm run deploy. Upload one file: ./scripts/with-cloudflare-env.sh npx wrangler r2 object put inneranimalmedia/static/dashboard/<file>.html --file=dashboard/<file>.html --content-type=text/html --remote -c wrangler.production.toml. Full doc: docs/AGENT_THEME_AND_COMPONENT_REFINEMENT.md.',
  'document',
  'design',
  NULL,
  NULL,
