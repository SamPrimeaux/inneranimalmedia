#!/usr/bin/env node
/**
 * D1 agentsam_memory iam_platform_context_router_v1 → Supabase agentsam_memory_oai3large_1536 + AGENTSAM_VECTORIZE_MEMORY.
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/sync_platform_context_router_memory_vector.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/sync_platform_context_router_memory_vector.mjs
 */
...
const MEMORY_KEY = 'iam_platform_context_router_v1';
...
const SCRIPT_KEY = 'sync_platform_context_router_memory_vector';
...
async function main() {
  ...
  const title = String(row.title || 'IAM platform context router').slice(0, 500);
  ...
  const metadata = {
    ...
    doc_type: 'platform_context_router',
    is_pinned: true,
  };
  const source = String(row.source || 'migration_639_platform_router').slice(0, 120);
