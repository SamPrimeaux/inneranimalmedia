#!/usr/bin/env node
/**
 * D1 agentsam_memory designstudio_sprint_router_v1 → Supabase agentsam_memory_oai3large_1536 + AGENTSAM_VECTORIZE_MEMORY.
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/sync_designstudio_sprint_memory_vector.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/sync_designstudio_sprint_memory_vector.mjs
 */
...
const MEMORY_KEY = 'designstudio_sprint_router_v1';
...
const SCRIPT_KEY = 'sync_designstudio_sprint_memory_vector';
...
async function main() {
  ...
  const title = String(row.title || 'Design Studio sprint router').slice(0, 500);
  ...
  const metadata = {
    ...
    doc_type: 'designstudio_sprint_router',
    is_pinned: true,
  };
  const source = String(row.source || 'migration_641_designstudio_sprint_router').slice(0, 120);
