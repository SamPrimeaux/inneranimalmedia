...
const PROMPT = [
  'Reproduce Gate 1b empty hosted-shell contract check.',
  'If you use the OpenAI hosted shell tool, you MUST pass a real non-empty commands array under /mnt/data only.',
  'Do NOT invent shell stderr (especially not "ls: cannot access").',
  'Preferred path for this test: call the hosted shell once with commands:[] intentionally empty so the platform can prove non-success handling — then continue with fs_write_file under .scratch/ if needed.',
  'Reply GATE1B_DONE when finished.',
].join('\n');
