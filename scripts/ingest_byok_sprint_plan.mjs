#!/usr/bin/env node
/**
 * BYOK sprint plan → agentsam_documents_oai3large_1536 + AGENTSAM_VECTORIZE_DOCUMENTS.
 * Manifest: docs/platform/byok-sprint.manifest.json
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_byok_sprint_plan.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_byok_sprint_plan.mjs
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_byok_sprint_plan.mjs --file docs/platform/byok-sprint-plan.md
 */
...
const SCRIPT_KEY = 'ingest_byok_sprint_plan';
const MANIFEST = 'docs/platform/byok-sprint.manifest.json';
const DOC_TYPE = 'byok_sprint_plan';
const TOPIC = 'byok_sprint';
...
async function main() {
  ...
  console.log(`ingest_byok_sprint_plan — ${args.dryRun ? 'DRY RUN' : 'LIVE'}`);
