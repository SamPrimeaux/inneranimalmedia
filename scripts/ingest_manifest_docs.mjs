#!/usr/bin/env node
/**
 * Manifest-driven platform docs → agentsam_documents_oai3large_1536 + AGENTSAM_VECTORIZE_DOCUMENTS.
 * Chunks by ## H2 section; idempotent via content_hash.
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_manifest_docs.mjs \
 *     --manifest docs/platform/platform-milestone-2026-06.manifest.json --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_manifest_docs.mjs \
 *     --manifest docs/platform/platform-milestone-2026-06.manifest.json
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_manifest_docs.mjs \
 *     --manifest docs/platform/iam-platform-docs.manifest.json
 */
...
const DEFAULT_MANIFEST = 'docs/platform/platform-milestone-2026-06.manifest.json';
...
function parseArgs(argv) {
  const out = { dryRun: false, file: null, manifest: DEFAULT_MANIFEST };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--dry-run') out.dryRun = true;
    else if (a === '--file' && argv[i + 1]) out.file = String(argv[++i]);
    else if (a.startsWith('--file=')) out.file = a.slice(7);
    else if (a === '--manifest' && argv[i + 1]) out.manifest = String(argv[++i]).replace(/^\//, '');
    else if (a.startsWith('--manifest=')) out.manifest = a.slice(11).replace(/^\//, '');
  }
  return out;
}

function scriptKeyFromManifest(manifestRel) {
  const base = basename(manifestRel, '.manifest.json').replace(/[^a-z0-9]+/gi, '_');
  return `ingest_manifest_docs_${base}`;
}
...
function loadManifest(root, manifestRel) {
  const p = join(root, manifestRel);
  if (!existsSync(p)) die(`Missing ${manifestRel}`);
  const data = JSON.parse(readFileSync(p, 'utf8'));
  const entries = Array.isArray(data.entries) ? data.entries : [];
  return {
    workspaceKey: data.workspace_key || 'ws_inneranimalmedia',
    d1ContextId: data.d1_context_id || D1_CONTEXT_ID,
    topic: data.topic || basename(manifestRel, '.manifest.json'),
    docType: data.doc_type || 'platform_manifest_doc',
    entries,
  };
}
...
function resolveFiles(root, args) {
  ...
  const { entries } = loadManifest(root, args.manifest);
  return entries
    .filter((e) => e?.file)
...
function buildRows(relPath, docTitle, meta, sections, workspaceUuid, gitSha, laneKey, manifestMeta) {
  const now = new Date().toISOString();
  const topic = meta.topic || manifestMeta?.topic || basename(relPath, '.md');
  const docSlug = basename(relPath, '.md');
  const projectKey = meta.project_key || PROJECT_KEY;
  const d1ContextId = meta.d1_context_id || manifestMeta?.d1ContextId || D1_CONTEXT_ID;
  const docType = meta.doc_type || manifestMeta?.docType || 'platform_manifest_doc';
...
async function ingestOneFile({ root, rel, workspaceUuid, d1Key, gitSha, dryRun, client, runId, scriptKey, manifestMeta }) {
  ...
  const pending = buildRows(rel, docTitle, meta, sections, workspaceUuid, gitSha, laneKey, manifestMeta);
      ...
      .map(({ row, embedding, pending: p }) => {
        ...
        return {
          ...
          metadata: {
            ...
            source_type: 'knowledge',
            doc_type: manifestMeta?.docType || 'platform_manifest_doc',
            project_key: PROJECT_KEY,
            topic: manifestMeta?.topic || 'platform_manifest',
          },
        };
      })
      .filter(Boolean);
  ...
  const runSyncChunkId = `run:${scriptKey}:${rel}`;
  writeVectorizeSyncReceipt({
    root,
...
async function main() {
  ...
  const manifestMeta = loadManifest(root, args.manifest);
  const scriptKey = scriptKeyFromManifest(args.manifest);
  const d1Key = String(process.env.D1_WORKSPACE_KEY || manifestMeta.workspaceKey).trim();
  const workspaceUuid =
  ...
  console.log(`ingest_manifest_docs — ${args.dryRun ? 'DRY RUN' : 'LIVE'}`);
  console.log(`run_id: ${runId}`);
  console.log(`manifest: ${args.manifest}`);
  console.log(`d1_context_id: ${manifestMeta.d1ContextId}`);
  ...
  try {
    ...
    console.log(`\nDone — ${totalChunks} total chunks from ${files.length} platform doc(s)`);
  } finally {
