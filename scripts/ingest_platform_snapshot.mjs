#!/usr/bin/env node
/**
 * IAM platform runtime docs → agentsam_documents_oai3large_1536 + AGENTSAM_VECTORIZE_DOCUMENTS.
 * Manifest: docs/platform/iam-platform-docs.manifest.json
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_platform_snapshot.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_platform_snapshot.mjs
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_platform_snapshot.mjs --file docs/platform/iam-platform-snapshot.md
 */
...
const SCRIPT_KEY = 'ingest_platform_snapshot';
const MANIFEST = 'docs/platform/iam-platform-docs.manifest.json';
  ...
  return sections.map((sec, i) => {
    ...
    return {
      ...
      metadata: {
        ...
        doc_type: meta.doc_type || 'platform_runtime_snapshot',
      },
    };
  });
}
...
async function main() {
  ...
  console.log(`ingest_platform_snapshot — ${args.dryRun ? 'DRY RUN' : 'LIVE'}`);
