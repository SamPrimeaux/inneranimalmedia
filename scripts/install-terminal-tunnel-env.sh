...
set -e
PTY_ENV="$HOME/iam-pty/.env"
mkdir -p "$HOME/iam-pty"
cat > "$PTY_ENV" <<EOF
# Generated by install-terminal-tunnel-env.sh (remote)
WORKER_URL=__WORKER_URL__
PTY_AUTH_TOKEN=__PTY_TOKEN__
TERMINAL_SECRET=__TERMINAL_SECRET__
...
REMOTE
  ...
  remote_script="${remote_script//__PTY_TOKEN__/$PTY_TOKEN}"
  remote_script="${remote_script//__TERMINAL_SECRET__/$TERMINAL_SECRET_VAL}"
