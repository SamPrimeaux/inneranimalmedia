...
sudo mkdir -p "$PTY_DIR"
sudo ln -sfn "$SSOT" "$PTY_DIR/.env.cloudflare"
sudo tee "$PTY_DIR/.env" >/dev/null <<EOF
...
EOF
sudo chmod 600 "$PTY_DIR/.env" "$PTY_DIR/.env.cloudflare" 2>/dev/null || sudo chmod 600 "$PTY_DIR/.env"
REMOTE
)"
