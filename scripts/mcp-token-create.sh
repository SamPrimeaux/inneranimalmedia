...
echo "Setting $SECRET_NAME on inneranimalmedia..."
echo "$TOKEN" | npx wrangler secret put "$SECRET_NAME" \
  --config wrangler.production.toml

echo "Setting $SECRET_NAME on inneranimalmedia-mcp-server..."
echo "$TOKEN" | npx wrangler secret put "$SECRET_NAME" \
  --name inneranimalmedia-mcp-server

echo "Writing hash to D1..."
