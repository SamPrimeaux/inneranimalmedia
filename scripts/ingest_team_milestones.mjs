#!/usr/bin/env node
/**
 * Curated team milestone markdown → agentsam_documents_oai3large_1536 + AGENTSAM_VECTORIZE_DOCUMENTS.
 * Reads docs/milestones/manifest.json — lightweight alternative to full codebase reindex.
 *
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_team_milestones.mjs --dry-run
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_team_milestones.mjs
 *   ./scripts/with-cloudflare-env.sh node scripts/ingest_team_milestones.mjs --file docs/milestones/2026-06-11-moviemode-progress.md
 */
...
const SCRIPT_KEY = 'ingest_team_milestones';
const MANIFEST = 'docs/milestones/manifest.json';
...
function resolveFiles(root, args) {
  ...
  return entries.filter((e) => e?.file).map((e) => ({
...
function buildRows(relPath, docTitle, meta, sections, workspaceUuid, gitSha, laneKey) {
  ...
  return sections.map((sec, i) => {
    ...
    const sourceRef = `milestones/${topic}#${i}`;
    return {
      ...
      metadata: {
        ...
        doc_type: meta.doc_type || 'team_milestone',
        milestone_date: meta.milestone_date || null,
        client_project_key: meta.client_project_key || null,
      },
    };
  });
}
...
async function ingestOneFile({ root, rel, workspaceUuid, d1Key, gitSha, dryRun, client, runId }) {
  ...
  console.log(`   ✓ ${savedRows.length} chunks → pgvector + Vectorize`);
  return { skipped: false, chunks: savedRows.length };
}
...
async function main() {
  ...
  console.log(`ingest_team_milestones — ${args.dryRun ? 'DRY RUN' : 'LIVE'}`);
  ...
  try {
    ...
    console.log(`\nDone — ${totalChunks} total chunks from ${files.length} milestone file(s)`);
  } finally {
