...
def build_readme(public_base: str, r2_prefix: str, bucket: str) -> str:
    generated = now_utc()
    public_learn_url = f"{public_base.rstrip('/')}/{r2_prefix.strip('/')}/"
    ...
    This folder is the source package for the `/learn/` area inside the `{bucket}` R2 bucket.
...
def write_readmes(readme: str) -> None:
    targets = [
        Path("learn/README.md"),
        Path("docs/learn/r2-course-library-readme.md"),
    ]
...
def upload_to_r2(bucket: str, r2_prefix: str, dry_run: bool = False) -> None:
    object_key = f"{bucket}/{r2_prefix.strip('/')}/README.md"
    cmd = [
        "npx",
        "wrangler",
        "r2",
...
def verify_from_r2(bucket: str, r2_prefix: str, dry_run: bool = False) -> None:
    out_file = "/tmp/learn_root_README.md"
    object_key = f"{bucket}/{r2_prefix.strip('/')}/README.md"
    ...
    if not dry_run:
        print("")
        print(f"Preview from {out_file}:")
        print("-" * 80)
        print(Path(out_file).read_text(encoding="utf-8").splitlines()[0:40])


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Write the Learning OS R2 course library README and optionally upload/verify it in R2."
    )
    ...
    parser.add_argument("--upload", action="store_true", help="Upload learn/README.md to R2.")
    parser.add_argument("--verify", action="store_true", help="Download learn/README.md back from R2 for verification.")
    parser.add_argument("--dry-run", action="store_true", help="Print R2 commands without executing them.")
    ...
    write_readmes(readme)

    if args.upload:
        print("")
        print("Uploading root Learning OS README to R2:")
        upload_to_r2(args.bucket, args.r2_prefix, dry_run=args.dry_run)

    if args.verify:
        print("")
        print("Verifying root Learning OS README from R2:")
        verify_from_r2(args.bucket, args.r2_prefix, dry_run=args.dry_run)

    print("")
    print("Next:")
    print("  git add learn/README.md docs/learn/r2-course-library-readme.md scripts/write_learn_area_readme.py")
    print('  git commit -m "docs: add Learning OS R2 course library README"')
    print("  git push")
    print("")
    print("Optional R2 upload + verify:")
    print("  python3 scripts/write_learn_area_readme.py --upload --verify")
