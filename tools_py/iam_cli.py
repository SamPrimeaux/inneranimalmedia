...
def _usage_events_where(interval: str) -> str:
    """
    agentsam_usage_events.created_at and agentsam_command_run.created_at are
    INTEGER unix-epoch (DEFAULT unixepoch()). datetime('now',...) returns TEXT
    and silently returns zero rows. Must use unixepoch('now',...).
    """
...
LEFT JOIN agentsam_workflows w ON w.workflow_key=c.workflow_key AND COALESCE(w.is_active,1)=1
...
FROM agentsam_commands
WHERE COALESCE(is_active,1)=1 AND COALESCE(failure_count,0)>0
...
FROM agentsam_commands
WHERE COALESCE(is_active,1)=1 AND COALESCE(avg_duration_ms,0)>0
...
def commands_doctor(args: argparse.Namespace) -> int:
    ...
    report = {"generated_at": stamp, "db": cfg.db, "config": cfg.config, "remote": cfg.remote, "diagnostics": diagnostics}
    json_path = out_dir / "commands-doctor.json"
    md_path = out_dir / "commands-doctor.md"
    write_json(json_path, report)
    lines = [
        "# Agent Sam Commands Doctor", "",
        f"Generated: `{stamp}`", f"DB: `{cfg.db}` remote=`{cfg.remote}`", "",
        "## Doctrine", "", "```text",
        "agentsam_commands = canonical command/action/capability registry",
        "agentsam_command_run = proposal/execution ledger — plain chat must not create rows",
        "created_at on command_run and usage_events = unix-epoch INTEGER (unixepoch() default)",
        "```", "",
    ]
    for name, rows in diagnostics.items():
        lines += [f"## {name.replace('_',' ').title()}", "", md_table(rows if isinstance(rows, list) else []), ""]
    write_text(md_path, "\n".join(lines))
    print("Wrote:")
...
def commands_export(args: argparse.Namespace) -> int:
    ...
    out_dir = ARTIFACTS / "commands" / stamp
    commands = d1_query("""
SELECT id, workspace_id, tenant_id, slug, display_name, description, mapped_command,
  ...
  is_global, route_key, use_count, success_count, failure_count, avg_duration_ms, last_used_at
FROM agentsam_commands WHERE COALESCE(is_active,1)=1
ORDER BY category, subcategory, risk_level, slug;
""", cfg)
    summary = d1_query("""
SELECT COALESCE(category,'uncategorized') AS category, COALESCE(subcategory,'general') AS subcategory,
  COALESCE(router_type,'unknown') AS router_type, COALESCE(task_type,'unknown') AS task_type,
  COALESCE(risk_level,'unknown') AS risk_level, requires_confirmation, requires_approval,
...
""", cfg)
    ...
    lines = ["# Agent Sam Command Catalog", "", f"Generated: `{stamp}`", f"Total active: `{len(commands)}`", "", "## Summary", "", md_table(summary), "", "## Commands By Group", ""]
    for group in sorted(groups):
        lines.append(f"### {group}")
        lines.append("")
        for cmd in groups[group]:
            lines += [
                f"#### {cmd.get('slug')} — {cmd.get('display_name')}",
                f"- id: `{cmd.get('id')}`",
                f"- risk: `{cmd.get('risk_level')}` approval=`{cmd.get('requires_approval')}` confirmation=`{cmd.get('requires_confirmation')}`",
                f"- router: `{cmd.get('router_type')}` tool=`{cmd.get('tool_key')}` workflow=`{cmd.get('workflow_key')}` route=`{cmd.get('route_key')}`",
                f"- mapped_command: `{cmd.get('mapped_command')}`",
            ]
...
def commands_pollution(args: argparse.Namespace) -> int:
    ...
    write_json(json_path, {"generated_at": stamp, "summary": summary, "recent_suspicious_rows": recent})
    write_text(md_path, "\n".join([
        "# Agent Sam Command Run Pollution Audit", "", f"Generated: `{stamp}`", "",
        "## Summary", "", md_table(summary), "",
        "## Recent Suspicious Rows", "", md_table(recent), "",
        "## Rule", "", "```text",
...
def excerpt_file(path: Path, terms: list[str], max_lines: int = 120) -> str:
    ...
    hits = [i for i, line in enumerate(lines) if any(t in line.lower() for t in lower_terms)]
    selected: list[int] = []
    if hits:
        for hit in hits[:20]:
...
def context_pack(args: argparse.Namespace) -> int:
    ...
    lines = [
        "# Agent Sam Context Pack", "", f"Generated: `{stamp}`", f"Task: `{query}`", "",
        "## Use This With A Coding Model", "", "```text",
...
FROM agentsam_usage_events WHERE {base_where}
GROUP BY provider, model, model_key ORDER BY cost_usd DESC LIMIT 50;
""",
        "by_tool": f"""
SELECT COALESCE(tool_name,'unknown') AS tool_name, COUNT(*) AS n,
  ...
  ROUND(AVG(COALESCE(duration_ms,0)),1) AS avg_duration_ms
FROM agentsam_usage_events WHERE {base_where}
GROUP BY tool_name ORDER BY cost_usd DESC LIMIT 50;
""",
        "failed_cost": f"""
...
GROUP BY status, tool_name, model ORDER BY cost_usd DESC LIMIT 50;
""",
    }
    ...
    for name, rows in data.items():
        lines += [f"## {name.replace('_',' ').title()}", "", md_table(rows), ""]
    write_text(md_path, "\n".join(lines))
    print("Wrote:")
    print_paths([md_path, json_path])
    return 0
...
<body>
  <main class=\"phone\" aria-label=\"iPhone 13 Pro prototype frame\">
    <header class=\"topbar\">
      <div><p class=\"eyebrow\">Agent Sam Prototype</p><h1>{args.name}</h1></div>
      <button class=\"pill\">Review</button>
    </header>
    <section class=\"search\"><input value=\"command doctor\" aria-label=\"Command search\" /></section>
    <section class=\"stack\" id=\"cards\"></section>
    <footer class=\"dock\">
      <button>Safe Read</button><button>Mutation</button><button>Danger</button>
    </footer>
  </main>
  <script type=\"module\">
    ...
    cards.innerHTML = data.commands.map(cmd => `
      <article class=\"card ${{cmd.risk}}\">
        <div><p class=\"eyebrow\">${{cmd.group}}</p><h2>${{cmd.name}}</h2><p>${{cmd.description}}</p></div>
        <span>${{cmd.risk}}</span>
      </article>`).join('');
  </script>
</body>
...
"""
    styles = """:root {
  color-scheme: dark;
  --panel: rgba(255,255,255,.075); --panel-strong: rgba(255,255,255,.12);
  --border: rgba(255,255,255,.12); --text: #f6f7fb; --muted: #a9afc2; --accent: #5a7df7;
  font-family: Inter, ui-sans-serif, system-ui, sans-serif;
}
* { box-sizing: border-box; }
body { margin: 0; min-height: 100vh; display: grid; place-items: center;
  background: radial-gradient(circle at 20% 0%, rgba(90,125,247,.22), transparent 32rem),
              linear-gradient(180deg, #111119, #08080b); color: var(--text); }
.phone { width: min(390px,100vw); min-height: min(844px,100vh);
  ...
  border: 1px solid var(--border); box-shadow: 0 24px 80px rgba(0,0,0,.45); }
.topbar,.card,.dock,.search input { border:1px solid var(--border); background:var(--panel);
...
h1,h2,p { margin:0; } h1 { font-size:22px; } h2 { font-size:16px; }
...
.card p:not(.eyebrow) { color:var(--muted); margin-top:6px; }
.card span { align-self:flex-start; border:1px solid var(--border); border-radius:999px; padding:5px 8px; font-size:12px; }
.card.critical { border-color:rgba(255,100,100,.55); } .card.high { border-color:rgba(255,180,90,.55); }
...
"""
    mock = {"commands": [
        ...
        {"group": "Mutation", "name": "cms-theme-audit", "risk": "medium", "description": "Run a controlled CMS theme audit."},
        {"group": "Danger Zone", "name": "/r2-bucket-delete", "risk": "critical", "description": "Destructive. Approval required."},
    ]}
...
def verify_patch(args: argparse.Namespace) -> int:
    checks = [["npm", "run", "build:vite-only"], ["python3", "tools_py/iam_cli.py", "commands", "doctor"]]
    if args.quick:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-env-wrapper", action="store_true")
    sub = parser.add_subparsers(dest="area", required=True)

    commands = sub.add_parser("commands")
    csub = commands.add_subparsers(dest="command_action", required=True)
    csub.add_parser("doctor").set_defaults(func=commands_doctor)
    csub.add_parser("export").set_defaults(func=commands_export)
    csub.add_parser("pollution").set_defaults(func=commands_pollution)
    smoke = csub.add_parser("smoke")
    ...
    report.add_argument("--last", default="7d")
